...
(function($){
	...
	//                on the keyboard, specify the Next button. If no button should be display, leave blank.
		...
		function resizePopup(popupPosition, callback)
		{
			...
			windowWidth = (('ontouchstart' in window) || window.DocumentTouch && document instanceof DocumentTouch) ? (window.innerWidth || $(window).width()) : $(window).width(); //retrieve current window width
			windowHeight = (('ontouchstart' in window) || window.DocumentTouch && document instanceof DocumentTouch) ? (window.innerHeight || $(window).height()) : $(window).height(); //retrieve current window height

			popupPosition.width = Math.min(windowWidth, popupPosition.width);
			popupPosition.height = Math.min(windowHeight, popupPosition.height);
			
			popup.parent().css('position', '');

			if(windowWidth <= options.maxWidth || windowHeight <= options.maxHeight){
				popup.parent().css('position', 'absolute');

				popupPosition.width = windowWidth;
				popupPosition.height = windowHeight;
				popupPosition.top	= $(window).scrollTop();
			...
			else
			{
				...
				if (!panelData.element)
				{
					...
					//{
						...
						media.element.attr('scrolling', (('ontouchstart' in window) || window.DocumentTouch && document instanceof DocumentTouch) ? 'no' : '').bind('load',ifmOnload);
		...
		function ifmOnload(){
			...
			var doc = $(this).contents().get(0),
				istouch = ((('ontouchstart' in window) || window.DocumentTouch && document instanceof DocumentTouch) ? 'no' : '');
			...
			if(istouch){
				...
				var ifmscroller = new iScroll(this.parentNode, {vScrollbar: true, hScrollbar: false, useTransform: istouch, scroller: (istouch ? doc.body : null) });
			...
			if(istouch){
				...
				if($.browser.opera || $.browser.mozilla || ($.browser.msie && $.browser.version >= 9)){
					$(doc).on('mousewheel.iscroll', function(e, which){
		...
		function onIfResize(){
			var jiframe = $('.yoxview_ja_slideshow:visible .yoxview_mediaPanel iframe:visible');
			
			if(jiframe.length){
				var ifmdoc = jiframe.contents(),
					scroller = jiframe.data('iscroll');
				...
				if(ifmdoc.length){
					...
					if(scroller){
						...
						if(!(jiframe.data('fixed') && height + padding <= nheight) || (height + padding < nheight) || (height - padding > nheight)){
							...
							scroller.scrollTo(0, Math.max(scroll, -nheight + ($('.yoxview_mediaPanel:visible').height() || 0)) );
							
							ifdelay = 500;
						}
					}

					iframeId = setTimeout(onIfResize, ifdelay);
				}
			}
		}

		function ifAutoResize(iframe){
