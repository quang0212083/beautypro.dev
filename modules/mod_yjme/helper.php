		...
		static function YJmeItems($params)
		{
			...
			foreach ( $load_items as $row ) {
				...
				$yj_get_items = array(
						...
						'item_url' 			=> $item_url,
						'item_intro' 		=> JString::substr(strip_tags($row->introtext,''.$allow_tags.''),0,$chars),
						'img_url' 			=> $img_url,
						'cat_title' 		=> htmlspecialchars($row->cattitle, ENT_QUOTES, 'UTF-8') ,
						'cat_url' 			=> $cat_url,
						'item_author' 		=> $row->username,
						'item_author_rn' 	=> $row->realname,
						'item_author_alias'	=> $row->created_by_alias,
						'author_url' 		=> $author_url,
						'item_date' 		=> JHTML::_('date', $row->created,JText::_('CREATEDATE')),
						'item_id'	 		=> $row->id,
						'item_fulltext'	 	=> $row->fulltext,
						'item_rating'	 	=> isset($row->ratingcount) ? round(($row->rating/$row->ratingcount)) : $row->rating,
					);
