 ...
 * @since       1.5
 */
abstract class ModRelatedItemsjoomirHelper
{
	public static function getList(&$params)
	{
		...
		if ($option == 'com_content' && $view == 'article' && $id)
		{
			...
			$db->setQuery($query);
			$joomirahmad=implode ( "," , $db->loadColumn() );

			if ($metakey = trim($joomirahmad))
			{
				...
				if (count($likes))
				{
					...
					$query->select($case_when)
						...
						->join('LEFT', '#__contentitem_tag_map AS tt ON tt.content_item_id = a.id')
						->where('a.id != ' . (int) $id)
						->where('a.state = 1')
						->where('a.access IN (' . $groups . ')');
					//$concat_string = $query->concatenate(array('","', ' REPLACE(a.metakey, ", ", ",")', ' ","'));
					$ahmadarrayjoomir = join(',',$likes);  
					$query->where('tt.tag_id IN ('.$ahmadarrayjoomir.')') //remove single space after commas in keywords)
