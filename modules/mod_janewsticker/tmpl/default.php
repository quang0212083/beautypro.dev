	...
	<div   id="<?php echo $moduleID; ?>" class="ja-headlines <?php echo $params->get('moduleclass_sfx');?>">
		...
		<?php foreach( $list as $index => $item ) : ?>
				<div class="ja-headlines-item <?php echo 'jahl-'.$animationType ;?>" style="visibility:<?php echo ($index !=0) ?'hidden':'visible' ?>">
					<a  title="<?php echo   modJANewStickerHelper::trimString(strip_tags($item->introtext), $descMaxChars); ?>" <?php echo $aClass ;?> href="<?php echo modJANewStickerHelper::getLink($item, $userRSS); ?>"><span><?php echo modJANewStickerHelper::trimString( $item->title, $titleMaxChars );?></span>
					</a> 
				</div>
