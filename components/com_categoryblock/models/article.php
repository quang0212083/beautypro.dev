<?php
/**
 * @version		$Id: article.php 21700 2012-06-28 04:32:41Z dextercowley $
 * @package		Joomla.Site
 ...
 * @subpackage	com_content
 */
class CategoryBlockModelArticle extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $_context = 'com_categoryblock.article';
	...
	protected function populateState()
	{
		...
		$params = $app->getParams('com_categoryblock');
	...
	public function &getItem($pk = null)
	{
		...
		if (!isset($this->_item[$pk])) {
			...
			try {
				...
				$query->select('c.title AS category_title, c.alias AS category_alias, c.access AS category_access, a.catid AS categoryid ');
				...
				$app = JFactory::getApplication('site');
				$params_global = $app->getParams('com_content');
				
				$data->params->merge($params_global);
				
				
				$categoryblockid=JRequest::getInt('cbprofile');
				if ($categoryblockid!=0)
				{
			
			
					$catid=$data->categoryid;
					$customitemid=JRequest::getInt('Itemid');
					require_once(JPATH_SITE.DS.'components'.DS.'com_categoryblock'.DS.'includes'.DS.'render.php');
					$cbm=new CategoryBlockMisc;
					$cbparams=$cbm->getSavedCategoryBlockParams($categoryblockid,$catid,$customitemid); //predefined proile
					$data->params->merge($cbparams);
					
					if($cbparams->get( 'allowcontentplugins' ))
				...
				if (!$user->get('guest')) {
					$userId	= $user->get('id');
					$asset	= 'com_categoryblock.article.'.$data->id;
