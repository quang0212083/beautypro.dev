...
class JCBPagination extends JObject
{
	/**
	...
	public function getCBLimitBox($columns)
	{
		...
		$html=str_replace('id="limit"','',$html);
	...
	protected function _buildDataObject()
	{
		...
		if($JoomlaVersionRelease==1.5)
			$limitstart_parameter='limitstart';
		else
			$limitstart_parameter='start';
		
		if (!$this->_viewall) {
				$data->all->base	= '0';
				$data->all->link	= JRoute::_($params.'&'.$this->prefix.$limitstart_parameter.'=');
		}
		...
		if ($this->get('pages.current') > 1)
		{
			...
			$data->start->base	= '0';
			$data->start->link	= JRoute::_($params.'&'.$this->prefix.$limitstart_parameter.'=0');
			$data->previous->base	= $page;
			$data->previous->link	= JRoute::_($params.'&'.$this->prefix.$limitstart_parameter.'='.$page);
		}
		...
		if ($this->get('pages.current') < $this->get('pages.total'))
		{
			...
			$data->next->base	= $next;
			$data->next->link	= JRoute::_($params.'&'.$this->prefix.$limitstart_parameter.'='.$next);
			$data->end->base	= $end;
			$data->end->link	= JRoute::_($params.'&'.$this->prefix.$limitstart_parameter.'='.$end);
		}
		...
		for ($i = $this->get('pages.start'); $i <= $stop; $i ++)
		{
			...
			if ($i != $this->get('pages.current') || $this->_viewall)
			{
				...
				$data->pages[$i]->link	= JRoute::_($params.'&'.$this->prefix.$limitstart_parameter.'='.$offset);
			}
		}
		return $data;
	}
