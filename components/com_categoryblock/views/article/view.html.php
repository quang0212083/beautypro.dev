<?php
/**
 * @version		$Id: view.html.php 21484 2012-06-08 00:57:51Z dextercowley $
...
//JHTML::addIncludePath(JPATH_SITE.DS.'components'.DS.'com_content'.DS.'helpers');
require_once(JPATH_SITE.DS.'components'.DS.'com_content'.DS.'helpers'.DS.'route.php');

class CategoryBlockViewArticle extends JViewLegacy
{
	protected $item;
	protected $params;
	protected $print;
	...
	function display($tpl = null)
	{
		...
		$this->state	= $this->get('State');// menu params
		$this->user		= $user;
		...
		$this->params	= $this->state->get('params');//Menu Params
		$active	= $app->getMenu()->getActive();//Article Params
		

		
		$temp	= clone ($this->params);//menu
		...
		$images=$cbm->getAllImages($item->text);
...
//echo '<br><hr>';
	...
	protected function _prepareDocument()
	{
		...
		if ($menu && ($menu->query['option'] != 'com_categoryblock' || $menu->query['view'] != 'article' || $id != $this->item->id))
		{
			...
			while ($category && ($menu->query['option'] != 'com_categoryblock' || $menu->query['view'] == 'article' || $id != $category->id) && $category->id > 1)
			{
