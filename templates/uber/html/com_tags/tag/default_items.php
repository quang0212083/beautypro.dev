	...
	<ul class="category media-list category-tags">
		<?php foreach ($items as $i => $item) : ?>
			...
			<?php if ($item->core_state == 0) : ?>
				<li class="media cat-list-row<?php echo $i % 2; ?>">
			<?php else: ?>
				<li class="cat-list-row<?php echo $i % 2; ?> media" >
			<?php endif; ?>

			<?php $images  = json_decode($item->core_images);?>

			<?php if ($this->params->get('tag_list_show_item_image', 1) == 1 && !empty($images->image_intro)) :?>
				<img src="<?php echo htmlspecialchars($images->image_intro);?>" alt="<?php echo htmlspecialchars($images->image_intro_alt); ?>" class="media-object pull-left">
			<?php endif; ?>
